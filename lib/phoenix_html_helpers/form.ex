defmodule PhoenixHTMLHelpers.Form do
  ...
  Forms may be used in two distinct scenarios:
    ...
    * with changeset data - when information to populate
      the form comes from an `Ecto.Changeset` changeset.
      The changeset holds rich information, which helps
  ...
  """
  @spec form_for(Phoenix.HTML.FormData.t(), String.t(), (Form.t() -> Phoenix.HTML.unsafe())) ::
          Phoenix.HTML.safe()
  @spec form_for(
          Phoenix.HTML.FormData.t(),
          String.t(),
          Keyword.t(),
          (Form.t() -> Phoenix.HTML.unsafe())
        ) ::
          Phoenix.HTML.safe()
  ...
  """
  @spec inputs_for(Form.t(), Form.field(), (Form.t() -> Phoenix.HTML.unsafe())) ::
          Phoenix.HTML.safe()
  @spec inputs_for(Form.t(), Form.field(), Keyword.t(), (Form.t() -> Phoenix.HTML.unsafe())) ::
          Phoenix.HTML.safe()
  ...
  """
  @spec input_type(Form.t(), Form.field()) :: atom
  def input_type(%{impl: impl, source: source} = form, field, mapping \\ @mapping)
      when is_atom(field) or is_binary(field) do
    type =
      if function_exported?(impl, :input_type, 3) do
